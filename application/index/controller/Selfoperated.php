...
class Selfoperated extends Common {
    ...
    public function getOrders() {
        ...
        $count = Db::name('mall_manage')->where($where)->count();
    ...
    public function handler() {
        ...
        switch (input('action')) {
            case 'enable':
                $info = Db::name('mall_manage')->where(['id' => $id, 'status' => '4'])->find();
                if (!$info) {
                    return errReturn(400,'不可启用的店铺类型');
                }
                $res = Db::name('mall_manage')->where(['id' => $id, 'status' => '4'])->update([
                    'status' => '2',
                ]);
                if ($res) {
                ...
                break;
            case 'disable':
                $info = Db::name('mall_manage')->where(['id' => $id, 'status' => '2'])->find();
                if (!$info) {
                    return errReturn(400,'不可禁用的店铺类型');
                }
                $res = Db::name('mall_manage')->where(['id' => $id, 'status' => '2'])->update([
                    'status' => '4'
                ]);
                if ($res) {
                ...
                break;
            case 'passed':
                $info = Db::name('mall_manage')->where(['id' => $id, 'status' => '0'])->find();
                if (!$info) {
                    return errReturn(400,'不可审核');
                }
                if ($this->roleId == '1') {
                    $res = Db::name('mall_manage')->where(['id' => $id, 'status' => '0'])->update([
                        'status' => '1'
                    ]);
                    if ($res) {
            ...
            case 'down':
                ...
                $res2 = Db::name('mall_manage')->where(['id' => $id, 'status' => '0'])->update([
                    'status' => '3'
                ]);
                if ($res2) {
    ...
    public function delete() {
        ...
        $res = Db::name('mall_manage')->where('id', input('id'))->delete();
    ...
    public function getorderlist() {
        ...
        foreach ($list as $k => $v) {
            $storeInfo = Db::name('mall_manage')->where('id', $v['store_id'])->find();
        ...
        $count = Db::name('mall_order')->where($where)->count();
