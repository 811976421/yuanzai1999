...
class Fredis{
    ...
    {
        $host = trim(isset($option['host']) ? $option['host'] : '127.0.0.1');
        $port = trim(isset($option['port']) ? $option['port'] : 6379);
        $auth = trim(isset($option['auth']) ? $option['auth'] : '');
        $index = trim(isset($option['index']) ? $option['index'] : 0);
...
//            $this->redisObj[$this->sn]->auth($auth);
    ...
    public function keys($patten) {
        return $this->redisObj[$this->sn]->keys($patten);;
    }
    /*------------------------------------2.start list结构----------------------------------------------------*/
    /**
     * 增，构建一个列表(先进后去，类似栈)
...
//          echo "$key - $value \n";
    ...
    public function spop($key) {
        ...
        if(!$re) return false;
        return $this->redisObj[$this->sn]->spop($key);
    }
    ...
    public function scard($key) {
        return $this->redisObj[$this->sn]->scard($key);
    }
    ...
    public function lock($key, $expTime) {
        //初步加锁
        $isLock = $this->redisObj[$this->sn]->setnx($key,time()+$expTime);
        if($isLock)
        {
            return true;
        }
        else
        {
            ...
            $val =$this->redisObj[$this->sn]->get($key);
            if($val&&$val<time())
            {
                $this->redisObj[$this->sn]->del($key);
            }
            return  $this->redisObj[$this->sn]->setnx($key,time()+$expTime);
        }
